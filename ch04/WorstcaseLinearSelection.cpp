...
Pair Select(vector<int>& arr, int lo, int hi, int k)
{
	...
	// for(int i = lo; i <= hi; i++)
	// 	SelectionSortPass(arr, i, hi, g == 1 ? 1 : 5);
	...
	// Pair x = TODO
	// int new_low = g == 1 ? lo + 2 : lo + 5 * (g / 2);
	// int new_high = g == 1 ? hi - 2 : hi - 5 * (g / 2);
	// Pair x = Select(arr, new_low, new_high, ceil(new_high - new_low + 1 / 2.0));
