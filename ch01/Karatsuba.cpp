...
string KaratsubaHelper(string str1, string str2, int level) // level은 디버깅용
{
	...
	string ab_mul_cd = KaratsubaHelper(a_plus_b, c_plus_d, level + 1);


	// 두 번째 항 만들기
	string sec_term = Subtract(ab_mul_cd, Add(ac, bd)); // (a + b)(c + d) - (ac + bd)
	// 문자열 뒤에 '0'을 추가해서 10^N를 O(N)으로 처리
	// 첫 번째 항 '0'추가
	ac.append(string((N - mid) * 2, '0'));
	// 두 번째 항 '0'추가
	sec_term.append(string((N - mid), '0'));

	// ac + ((a + b)(c + d) - ac - bc)
	string result = Add(ac, sec_term);
